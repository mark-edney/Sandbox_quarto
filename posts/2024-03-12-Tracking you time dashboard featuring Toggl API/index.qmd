...
df2 = df.groupby('project')['dur'].sum()/52
df2['Sleep']= 8*7
df2['Spare'] = 24*7 - df2.sum()

fig3 = px.pie(df2, values='dur', names = df2.index, title='Average time spent every month')
# App layout
app.layout = html.Div([
...
def update_graph(hist):
    fig = px.histogram(df, x='project', y='dur', histfunc=hist)
    return fig
